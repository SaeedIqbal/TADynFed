...
def tissue_specific_classification_loss(outputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
    """
    outputs: [Batch x Classes]
    targets: [Batch x Classes] (binary one-hot)
    """
    probs = F.softmax(outputs, dim=1)
    return -torch.mean(targets * torch.log(probs + 1e-8))
...
def contrastive_modality_loss(anchor: torch.Tensor, positive: torch.Tensor, negative: torch.Tensor, margin: float = 0.5) -> torch.Tensor:
